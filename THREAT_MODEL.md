...
| Context-dependent | Application mistakes Murmur output/seed for authentication, collision-resistant identity or concealment of predictable inputs | Downstream security decision relies on a property not offered by the library | Forged identity/decision integrity, or recovery of matching low-entropy inputs by candidate enumeration, if that reliance is real | README explicitly says non-cryptographic | Use a cryptographic primitive with the required security contract | `README.md:2`, `smhasher.cpp:27` |
...
A clean assessment separates wrapper defects from host misuse and already-authorized build authority. Changing the installed extension with an account that already controls application binaries is not a new privilege gain. Knowing a hash does not conceal a low-entropy input: an observer who knows the variant and seed can enumerate candidate passwords, tokens or other predictable values and compare hashes without an implementation defect. Higher input entropy can make guessing less practical, but Murmur supplies no cryptographic preimage/confidentiality guarantee. This is downstream misuse distinct from access to unrelated process memory, which still requires a separately demonstrated memory failure (`README.md:2`, `smhasher.cpp:27-38`).
